...
export default function PlaygroundMain() {
  ...
  const handleLangChange = (lang: LanguageKey) => {
    ...
    if (terminalRef.current) {
      ...
      terminalRef.current.writeln(
        `\x1b[1;36m→ Language switched to ${LANGUAGES[lang].name}\x1b[0m`,
      );
      terminalRef.current.writeln(
        `\x1b[90mReady to execute. Press Run or Ctrl+Enter.\x1b[0m`,
      );
      terminalRef.current.writeln("");
    }
  };
  ...
  const resetCode = () => {
    ...
    if (terminalRef.current) {
      ...
      terminalRef.current.writeln("");
      terminalRef.current.writeln(
        `\x1b[1;33m↺ Code reset to default template\x1b[0m`,
      );
  ...
  const runCode = useCallback(async () => {
    ...
    if (terminalRef.current) {
      terminalRef.current.reset();
      terminalRef.current.writeln(
        `\x1b[1;33m⚡ Running ${currentLang.name}...\x1b[0m`,
      );
      terminalRef.current.writeln("");
    ...
    try {
      ...
      if (terminalRef.current) {
        if (result.run) {
          ...
          terminalRef.current.writeln("");
          terminalRef.current.writeln(
            `\x1b[1;${statusColor}m${statusIcon} Exit Code: ${exitCode}\x1b[0m \x1b[90m(Execution time: ${executionTime}ms)\x1b[0m`,
          );
      ...
      if (terminalRef.current) {
        terminalRef.current.writeln("");
        terminalRef.current.writeln(
          `\x1b[1;31m✗ Error: ${error instanceof Error ? error.message : String(error)}\x1b[0m`,
        );
        terminalRef.current.writeln(
          `\x1b[90mPlease check your internet connection or try again.\x1b[0m`,
        );
      }
    } finally {
      setIsRunning(false);
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-w-0">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-[#1a1a1a] border border-white/20 rounded-xl shadow-2xl z-50 py-2 backdrop-blur-xl">
                    ...
                    {executionHistory
                      ...
                      .map((item, i) => (
                        <div
                          key={i}
                          className="px-3 py-2 text-xs flex items-center justify-between hover:bg-white/5 transition-colors"
                        >
                          <span className="text-gray-300 font-medium">
                            {item.language}
                          </span>
                          <span
                            className={`font-semibold ${item.success ? "text-green-400" : "text-red-400"}`}
                          >
          ...
          >
            {/* Editor - Dark theme */}
            <div
              className={`relative min-h-0 border-b lg:border-b-0 lg:border-r border-white/10 bg-[#050505]
            ${
              layoutMode === "editor-focus"
                ? "flex-[3]"
