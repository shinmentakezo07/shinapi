...
func (r *AdminFeaturesRepo) ListAnnouncements(ctx context.Context) ([]domain.Announcement, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.Title, &a.Body, &a.Priority, &a.TargetType, &a.TargetIDs, &a.StartsAt, &a.EndsAt, &a.ShowInApp, &a.SendEmail, &a.CreatedBy, &a.CreatedAt); err != nil {
...
func (r *AdminFeaturesRepo) ListPromoCodes(ctx context.Context) ([]domain.PromoCode, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Code, &p.Type, &p.Value, &p.MaxUses, &p.CurrentUses, &p.MinPurchase, &p.ExpiresAt, &p.IsActive, &p.CreatedBy, &p.CreatedAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
...
func (r *AdminFeaturesRepo) GetPromoRedemptions(ctx context.Context, promoID string) ([]domain.PromoRedemption, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&red.ID, &red.PromoID, &red.UserID, &red.Discount, &red.CreditsAwarded, &red.RedeemedAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
...
func (r *AdminFeaturesRepo) ListScheduledReports(ctx context.Context) ([]domain.ScheduledReport, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.Frequency, &s.Format, &s.Sections, &s.Recipients, &s.NextSendAt, &s.LastSentAt, &s.IsActive, &s.CreatedAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
...
func (r *AdminFeaturesRepo) ListChangelog(ctx context.Context, drafts bool) ([]domain.ChangelogEntry, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.Title, &e.Body, &e.Version, &e.Type, &e.PublishedAt, &e.IsDraft, &e.CreatedBy, &e.CreatedAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
...
func (r *AdminFeaturesRepo) ListSSOConfigs(ctx context.Context) ([]domain.SSOConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list sso: %w", err)
	}
	defer rows.Close()
	var configs []domain.SSOConfig
	for rows.Next() {
		var c domain.SSOConfig
		if err := rows.Scan(&c.ID, &c.Provider, &c.Label, &c.Issuer, &c.ClientID, &c.AllowedDomains, &c.AutoProvision, &c.DefaultRole, &c.IsActive, &c.CreatedAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
