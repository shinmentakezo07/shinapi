...
const drawerVariants = {
  ...
  visible: { x: 0, opacity: 1, transition: { type: "spring" as const, stiffness: 280, damping: 28 } },
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="p-1.5 rounded-md hover:bg-white/[0.06] transition-colors group" aria-label={copied ? "Copied" : "Copy to clipboard"}>
  ...
  return (
    <div className="flex items-center justify-between py-3 px-2 -mx-2 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.015] rounded-lg transition-colors group">
      ...
      <div className="flex items-center gap-2 min-w-0">
        <span className={`text-sm text-white truncate max-w-[200px] ${mono ? "font-mono" : ""}`}>{value}</span>
  ...
  return (
    <AnimatePresence>
      {log && (
        <>
          {/* Backdrop */}
          <motion.div variants={backdropVariants} initial="hidden" animate="visible" exit="exit" onClick={onClose} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" />
          ...
          >
            ...
            <div className="sticky top-0 z-10 bg-[#0c0c0e]/90 backdrop-blur-xl border-b border-white/[0.06] px-6 py-5 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-white tracking-tight">Log Details</h2>
                <p className="text-[11px] text-slate-600 font-mono mt-0.5">Request metadata</p>
              </div>
              <motion.button whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} onClick={onClose} className="p-2 rounded-lg hover:bg-white/[0.06] transition-colors" aria-label="Close drawer">
            ...
            <div className="p-6 space-y-7">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2.5 rounded-xl ${isSuccess ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
                    <StatusIcon className={`w-5 h-5 ${isSuccess ? "text-emerald-400" : "text-red-400"}`} />
                  </div>
                  <div>
                    <p className={`text-sm font-semibold ${isSuccess ? "text-emerald-400" : "text-red-400"}`}>{statusText}</p>
                    {log.errorMessage && (
                      <p className="text-xs text-red-400/60 mt-1 font-mono break-all">{log.errorMessage}</p>
                    )}
                  </div>
                </div>
              </motion.div>
              ...
              <Section icon={Zap} title="Usage & Cost">
                ...
                <DetailRow icon={Zap} label="Total Tokens" value={(log.inputTokens + log.outputTokens).toLocaleString()} mono />
                <div className="flex items-center justify-between py-3 px-2 -mx-2 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.015] rounded-lg transition-colors">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-[11px] text-slate-500 uppercase tracking-widest font-medium">Latency</span>
                  </div>
                  <span className="text-sm font-mono text-white">{log.latency}ms</span>
                </div>
                <div className="flex items-center justify-between py-3 px-2 -mx-2 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.015] rounded-lg transition-colors">
                  <div className="flex items-center gap-2.5">
                    <Clock className="w-4 h-4 text-slate-600" />
                    <span className="text-[11px] text-slate-500 uppercase tracking-widest font-medium">Cost</span>
                  </div>
                  <span className="text-sm font-mono text-emerald-400">${(log.cost / 100000).toFixed(6)}</span>
                </div>
              </Section>
              ...
              <Section icon={KeyRound} title="Identifiers">
                ...
                {log.apiKeyId && <DetailRow icon={KeyRound} label="API Key ID" value={log.apiKeyId} mono copyable />}
                <DetailRow icon={User} label="User ID" value={log.userId} mono copyable />
              </Section>

              {/* Timestamp */}
              <Section icon={Clock} title="Timestamp">
                <DetailRow icon={Clock} label="Created" value={new Date(log.createdAt).toLocaleString()} mono />
              </Section>
            </div>
          </motion.div>
        </>
...
function Section({
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/[0.015] rounded-xl border border-white/[0.04] p-1">{children}</div>
    </motion.div>
  );
}
