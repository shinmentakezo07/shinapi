...
function HUDOverlay() {
  return (
    <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
      ...
      <div className="absolute top-12 right-28 text-[10px] font-mono text-white/20 hidden lg:block tracking-widest">AUTH.V.3.0 // GATEWAY</div>
      <div className="absolute bottom-12 left-28 text-[10px] font-mono text-white/20 hidden lg:block tracking-widest">SESSION: PENDING</div>
    </div>
  );
}
...
function TerminalInput({
  id, name, type = "text", placeholder, required, autoComplete, value, onChange, icon: Icon, error, inputRef,
}: {
  id: string; name: string; type?: string; placeholder: string; required?: boolean; autoComplete?: string;
  value?: string; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void;
  icon: React.ElementType; error?: string; inputRef?: React.Ref<HTMLInputElement>;
}) {
...
function AuthTerminal() {
  ...
  const src = `curl -X POST https://api.yapapa.ai/v1/auth/login \\\n  -H "Content-Type: application/json" \\\n  -d '{ "email": "you@co.ai", "password": "***" }'\n\n# 200 OK\n# Token: eyJhbG...••••\n# Expires: 24h`;
  useEffect(() => {
    let i = 0;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4, duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="relative mt-10">
      ...
      <div className="relative bg-[#060606] border border-white/[0.06] rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.04] bg-white/[0.01]">
          ...
          <span className="text-[9px] font-mono text-white/20">auth.sh</span>
        </div>
...
function SubmitButton() {
  ...
  return (
    <CyberButton type="submit" primary disabled={pending} className="w-full">
      {pending ? <Loader2 className="w-4 h-4 animate-spin" /> : <><span>Access Gateway</span><ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" /></>}
    </CyberButton>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#050505] selection:bg-primary/30 selection:text-white relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex flex-col lg:flex-row">
        ...
        <div className="lg:w-[45%] flex flex-col justify-between p-8 lg:p-12 lg:py-16 relative">
          ...
          <div className="flex-1 flex flex-col justify-center py-12 lg:py-0 max-w-md">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 px-2.5 py-1 mb-6 rounded bg-white/5 border border-white/10 w-fit">
              <div className="flex items-center gap-1.5 px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 text-[9px] font-mono font-bold"><KeyRound className="w-2.5 h-2.5" />GATEWAY</div>
              <span className="text-[11px] font-mono text-gray-400">Secure API access portal</span>
            </motion.div>

            <h2 className="text-5xl lg:text-[4.5rem] font-bold tracking-tighter leading-[0.85] text-white mb-6">
              <TypewriterText text="Welcome" delay={0.3} /><br />
              <GlitchText text="BACK" className="text-5xl lg:text-[4.5rem] font-black" />
            </h2>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0 }} className="text-base text-gray-400 leading-relaxed mb-8 max-w-sm">
              Sign in to manage your API keys, monitor usage, and configure your gateway. <span className="text-white font-medium">All your models</span>, one dashboard.
            </motion.p>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="flex gap-8 mb-8">
              <AnimatedStat value="100+" label="Models" delay={1.3} />
              <AnimatedStat value="50K+" label="Developers" delay={1.4} />
              <AnimatedStat value="99.9%" label="Uptime" delay={1.5} />
            </motion.div>

            {/* Feature list — not cards */}
            <div className="space-y-0 relative">
              <div className="absolute left-[11px] top-3 bottom-3 w-[1px] bg-gradient-to-b from-blue-500/30 via-emerald-500/30 to-violet-500/30" />

              {[
                { label: "Real-time analytics", detail: "Track every request, every token", color: "text-blue-400", dot: "bg-blue-500" },
                { label: "Multi-provider routing", detail: "OpenAI, Claude, Gemini, and more", color: "text-emerald-400", dot: "bg-emerald-500" },
                { label: "Team management", detail: "Orgs, roles, and API key scoping", color: "text-violet-400", dot: "bg-violet-500" },
              ].map((item, i) => (
                <motion.div
                  key={item.label}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 1.6 + i * 0.12, ease: [0.16, 1, 0.3, 1] }}
                  className="flex items-center gap-4 py-2.5 relative"
                >
                  <div className={`w-[22px] h-[22px] rounded-full border border-white/10 flex items-center justify-center shrink-0 z-10 bg-[#050505]`}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0 }} className="text-[10px] font-mono text-gray-700 tracking-wider mt-8 lg:mt-0">
        ...
        {/* RIGHT: Form column — no card wrapper */}
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[440px]">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-mono font-bold tracking-widest text-blue-400">GATEWAY ACCESS</span>
              </div>
              <h1 className="text-3xl font-bold text-white tracking-tight">Sign In</h1>
              <p className="text-gray-400 text-sm mt-1">Access your dashboard and API keys.</p>
            </div>
            ...
            <form action={dispatch} className="space-y-3">
              <TerminalInput id="email" name="email" type="email" placeholder="jane@company.com" required autoComplete="email" icon={Mail} error={errorMessage} inputRef={emailRef} value={email} onChange={(e) => setEmail(e.target.value)} />
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-3">
                    <Link href="/forgot-password" className="text-[10px] font-mono text-blue-400 hover:text-blue-300 transition-colors">FORGOT?</Link>
                ...
                <TerminalInput id="password" name="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" required autoComplete="current-password" icon={Lock} />
              </div>

              <AnimatePresence>
                {errorMessage && (
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-3 rounded-lg text-sm font-mono bg-red-500/10 border border-red-500/20 text-red-400">
                    {errorMessage}
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="mt-6 flex items-center justify-between">
              <p className="text-[13px] text-gray-500">
                New here?{" "}
                <Link href="/signup" className="text-blue-400 hover:text-blue-300 transition-colors font-medium">Create Account</Link>
              </p>
